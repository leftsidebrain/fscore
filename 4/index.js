...
async function home(req, res) {
  try {
    ...
    const query = `SELECT * FROM "tb_collections" `;
    const obj = await sequelize.query(query, { type: QueryTypes.SELECT });
...
async function register(req, res) {
  ...
  bcrypt.hash(password, 10, async (err, hash) => {
    ...
    const query = `INSERT INTO "tb_users"(email,username,password,"createdAt","updatedAt") VALUES ('${email}','${name}','${hash}',now(),now())`;
...
async function addTask(req, res) {
  ...
  const query = `INSERT INTO "tb_tasks" (to_do,is_done,collections_id,"createdAt","updatedAt") VALUES ('${to_do}','${v}','${id}',NOW(),NOW()) `;
  await sequelize.query(query, { type: QueryTypes.INSERT });
...
async function editView(req, res) {
  const { id } = req.params;
  const query = `SELECT "tb_collections".name,"tb_tasks".* FROM "tb_collections" 
  RIGHT JOIN "tb_tasks" ON "tb_collections".id = "tb_tasks".collections_id 
  WHERE "tb_collections".id = ${id} ORDER BY "tb_collections".id DESC`;
  const query2 = `SELECT name,id FROM "tb_collections" WHERE id = ${id}`;
  const obj = await sequelize.query(query, { type: QueryTypes.SELECT });
...
async function addColection(req, res) {
  try {
    ...
    const query = `INSERT INTO "tb_collections" (name,user_id,"createdAt","updatedAt") VALUES ('${name}','${id_user}',NOW(),NOW())`;
...
async function deleteCollections(req, res) {
  const { id } = req.params;
  const query = `DELETE FROM "tb_collections" WHERE id=${id}`;
  const query2 = `DELETE FROM "tb_tasks" WHERE collections_id=${id}`;
...
app.get("/edit_todo/:id", todoView);
function todoView(params) {
  const { id } = req.params;
  req.render("edit-todo", { id });
}
app.get("/edit-todo/:id", todoView);
async function todoView(req, res) {
  const { id } = req.params;
  const query = `SELECT * FROM "tb_tasks" WHERE id =${id}`;
  const obj = await sequelize.query(query, { type: QueryTypes.SELECT });
  console.log("🚀 ~ todoView ~ obj:", obj);
  res.render("edit-todo", { data: obj[0] });
}
...
async function updateTodo(req, res) {
  ...
  const query = `UPDATE "tb_tasks" SET to_do='${to_do}',is_done='${v}',collections_id='${collections_id}',"createdAt"=NOW(),"updatedAt"=NOW() WHERE id=${id}`;
  await sequelize.query(query, { type: QueryTypes.SELECT });
